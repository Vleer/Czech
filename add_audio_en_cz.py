"""
Add Czech TTS to the English→Czech deck. Injects [sound:...] into the back (Czech side).

Run after make_anki_cards_en_cz.py. Uses output/<dataset>/audio/ (same as Czech→English).
Usage: python add_audio_en_cz.py [dataset]
"""
import asyncio
import csv
import hashlib
...
INPUT_TSV = paths.en_cz_tsv(DATASET)
OUTPUT_TSV = paths.en_cz_audio_tsv(DATASET)
...
def _inject_sound_into_back(back: str, audio_name: str) -> str:
    sound_tag = f"[sound:{audio_name}]"
    if "[sound:" in back:
        back = re.sub(r"\[sound:[^\]]+\.mp3\]\s*(?:<br><br>)?", "", back)
    marker = "<br><br>"
    idx = back.find(marker)
    if idx == -1:
        return f"{sound_tag}<br><br>{back}"
    insert_at = idx + len(marker)
    return back[:insert_at] + f"{sound_tag}<br><br>" + back[insert_at:]
...
async def main() -> None:
    ...
    if not INPUT_TSV.exists():
        raise SystemExit(f"Input TSV not found: {INPUT_TSV}. Run make_anki_cards_en_cz.py first.")
    ...
    print(f"Generating TTS for {len(unique_sentences)} Czech example sentences (back side)...")
    ...
    with OUTPUT_TSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for i, (front, back) in enumerate(tsv_rows):
            if i < len(rows):
                ...
                back = _inject_sound_into_back(back, audio_name)
